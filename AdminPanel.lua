...
local my_dialog = {
	...
	
	{
        ...
        onclick = function() deff = not deff 
			sampAddChatMessage('{FF0000}Никита запретил использовать данную функцию!!')  	
	    end
...
function main()
	...
	--	if not nop then 
	--	sampAddChatMessage('{00FF00}[Nop]: {FFFFFF}Выключен!', -1)
	--	else
	--	sampAddChatMessage('{00FF00}[Nop]: {FFFFFF}Включен!', -1)
	--	end
	end) 
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(updateIni.info.vers) > script_vers then
                sampAddChatMessage("{00FF00}[Admin Panel ETRP]: {ff0000}Обнаруженно Обновление: {FFFFFF}Новая Версия: " .. updateIni.info.vers_text, -1)
                update_state = true
            end
    ...
    while true do
        ...
        if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    sampAddChatMessage("{00FF00}[Admin Panel ETRP]{FFFFFF}:Скрипт успешно обновлен", -1)
                    thisScript():reload()
                end             
            end)
            break
